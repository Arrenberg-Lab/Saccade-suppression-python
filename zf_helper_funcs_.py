...
def florian_model_shuffle_parameters(nfilters, rsl, gfsf, gfsz, gfph, jsigma, img=None, params=None, macaque=False):
    """
    ...
    img: float, optional
        The stimulus image. Kept in the function just for backwards compatibility.
    ...
    xloc = xloc % (xend*rsl) #for center values bigger than 360° in azimuth, wrap the index around and get the
...
def macaque_xi_estimation(sacarr, onset, offset):
    ...
    for idx in range(sacarr.shape[0]):
        ...
        if len(postsaccade) < 20: #same story
            ...
            xi.append((postsaccade-postfit)**2)
    xi = np.array([a for b in xi for a in b])
    xi = np.sqrt(xi[~np.isnan(xi)])
...
def calculate_s_xi(xi):
    ...
    s_xipercentiled = np.std(xi[xi<np.percentile(xi,99)]) 
...
def macaque_eps_estimation(sacarr, xi, onset, offset, ucutoff= 0.01, RMSfac=0.5, onsrem = 5, offsrem=150, fplot=False, nplot=False):
    ...
    for idx in range(sacarr.shape[0]):
        ...
        varxi = np.var(xi[xi<np.percentile(xi,99)])
    ...
    nicesacidxs = np.where(RMSs<=np.mean(RMSs)-RMSfac*np.std(RMSs))[0]
    if nplot == True:
    ...
    epsl = np.array(epsl)
    epsl = np.array([a for b in epsl[nicesacidxs] for a in b])
    ul = np.array(ul)
    ul = np.array([a for b in ul[nicesacidxs] for a in b])
    ul = ul[~np.isnan(epsl) & ~np.isinf(epsl)]
    epsl = epsl[~np.isnan(epsl) & ~np.isinf(epsl)]
    
    #bin u values and epsilon values
    ubins = np.linspace(np.min(ul), np.max(ul), 100)
    binnedu = [[] for a in range(len(ubins))]
    binnedeps = [[] for a in range(len(ubins))]
    
    #sort u and eps into bins
    for idx, u in enumerate(ul):
        ubin = np.where(ubins<=u)[0][-1]
        binnedu[ubin].append(u)
        binnedeps[ubin].append(epsl[idx])
    
    #find average s_eps per bin and then average over all
    epsperbin = np.array([np.mean(a) for a in binnedeps])
    epsperbin = epsperbin[ubins>ucutoff]
    epsperbin = epsperbin[~np.isnan(epsperbin)]
    s_eps = np.mean(epsperbin)
    return ul, epsl, s_eps
...
class cylindrical_random_pix_stimulus:
    ...
    #!!!TODO: 1D version of the stimulus
...
def test_bar_stimulus_1D(rsl, shiftmag, tdur, barwidth, spbar, fps, cf):
    """
    Move a 1D stimulus bar.

    Parameters
    ----------
    rsl : int
        Resoultion in pixel per degrees.
    shiftmag : float
        Shift magnitude of the stimulus in degrees.
    tdur : float
        Duration of the stimulus in seconds.
    barwidth : float
        Width of the bar in degrees.
    spbar : float
        Starting azimuth of the bar in degrees.
    fps : int
        Number of frames per second.
    cf : int
        Current frame number for the given stimulus.    
    ...
    starr : 1-D array
        Stimulus array.    
    """
    azarr = np.linspace(-180,180, 360*rsl) #azimuth array
    bwpx = int(barwidth*rsl) #width of the bar in pixels
    barr = np.ones(bwpx) #bar array
    starr = np.zeros(azarr.shape) #stimulus array
    spidx = np.argmin(np.abs(azarr-spbar)) #index of the starting position
    shpf = int(shiftmag*rsl / (tdur*fps)) #shift magnitude per frame in pixels
    
    if shpf == 0:
        warnings.warn('Desired shift is smaller than the resolution allows, thus shift is set to 1 pixel.')
        shpf = 1
    
    frametot = int(tdur*fps)
    print(cf, frametot)
    if cf > frametot:
        warnings.warn('Given frame is above the total number of stimulus frames, last stimulus frame is returned.')
        cf = frametot
    elif (cf == frametot) & (cf*shpf != shiftmag*rsl):
        warnings.warn('Desired shift magnitude cannot be achieved with the given settings and constant bar speed.' + 
                      'The shift in last frame is thus compensated as much as the resolution allows.')
        shpf = 1
        cf = int(shiftmag*rsl) 
    
    starr[spidx+(cf*shpf):spidx+(cf*shpf)+bwpx] = barr
    return azarr, starr
...
def crop_rf_geographic(azidx, elevidx, degex, rsl, radiusfac=1, macaque=False):
    """
    ...
    if macaque == False:
        ...
        gaz, gel = np.meshgrid(np.linspace(-90,90,np.int(180*rsl)), np.linspace(90,-90,np.int(180*rsl)))
...
def gaussian_rf_geog(sf, sz, azidx, elevidx, rsl, radiusfac=1, verbose=False, macaque=False):
    """
    ...
    #Generate RF mask
    maskparams = [azidx, elevidx, sz, rsl, radiusfac]
    rfarr, rfcent, rfcentcar = crop_rf_geographic(*maskparams, macaque=macaque)
...
def gaussian_rf_geog_1D(sf, sz, azidx, rsl):
    """
    Generate Difference of Gaussian (DoG) or Gaussian profile for the receptive field.
    ...
    filterimg : 2-D array
        Image matrix of the filter array.
    """
    azarr = np.linspace(0, 360, int(360*rsl)) #azimuth array (with elevation being zero all the time)
    rds = sz/2 #radius in degrees
    rdspx = np.round(rds*rsl).astype(int)
    wl = 1/sf #spatial frequency wavelength in degrees
    stdg = wl/6 #set the standard deviation to wl/4, this is in degrees.
    
    #generate a Gaussian, which you will then insert according to azarr
    x = np.linspace(-rds, rds, 2*rdspx) #initial Gaussian is centered at zero
    gauss = 1/(stdg*np.sqrt(2*np.pi)) * np.exp(-0.5*(x/stdg)**2)
    gauss /= np.sum(gauss) #normalize to unit area

    rfarr = np.zeros(len(azarr))
    
    start = azidx - rdspx
    end = azidx + rdspx
    #print(start, end, gauss.shape, int(rds*rsl), azidx)
    
    if start < 0:
        rfarr[start+360*rsl:] = gauss[:np.abs(start)]
        rfarr[:end] = gauss[np.abs(start):]        
    elif end > 360*rsl:
        rfarr[start:] = gauss[:360*rsl-start]
        rfarr[:end-360*rsl] = gauss[360*rsl-start:]
    else:
        rfarr[start:end] = gauss
    return azarr, rfarr
