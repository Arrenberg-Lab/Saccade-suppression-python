...
#Choose circle center in geographic coordinates
elevidx, azidx = np.array((90,180)) * rsl #NOTE THAT 1st dimension is elevation.
gccent = np.array([gaz[elevidx,azidx], gel[elevidx,azidx]]) #azimuth and elevation
#Find a way to transform the radius in degrees to a value in cartesian: lets say you want your circle to have
#30° radius. then you can find that value in geograhic coordinates (one edge of the circle away from center) and 
#find the corresponding radius in cartesian coordinates. This is important since unit vector is not always the same
#in geographic coordinates.
grc = 50 #radius of the circle in geographic coordinates [degrees]
#find the approximately closest sampled point fulfilling r=grc
addidx = grc*rsl #index to be added to find the edge of the circle in geographic coordinates
...
cyf = cy.flatten()
czf = cz.flatten()
circarr, __, __ = hlp.crop_rf_geographic(*(np.array((180,90)) * rsl), 10, rsl, radiusfac=1)
fig = plt.figure()
ax = fig.add_subplot(121)
ax.imshow(circarr, extent=[-180,180,-90,90])
ax = fig.add_subplot(122, projection='3d')
ax.plot(cxf,cyf,czf, 'k.', label='Whole field', alpha= 0.2)
...
filterimg = hlp.generate_RF_geographic(sf, sz, 45, 200, 50, rsl, radiusfac=1)
